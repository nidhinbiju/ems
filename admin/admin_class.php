...
Class Action {
	...
	function save_user() {
		...
		$establishment_id = isset($establishment_id) ? $this->db->real_escape_string($establishment_id) : 0;
		$id = isset($id) ? $this->db->real_escape_string($id) : '';
	
		// Check if username already exists
		$existing_user_query = $this->db->query("SELECT * FROM users WHERE username = '$username' AND id != '$id'");
